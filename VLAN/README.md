...
VLAN（Virtual LAN）は、物理的な接続とは無関係に、**ブロードキャストドメインを論理的に分割**し、ネットワークのセキュリティと管理性を高める技術です。

しかし、異なるVLANに属するPC同士は、**論理的に分離された別のネットワーク**と見なされるため、L2スイッチ経由ではそのままでは直接通信できません。

そこで今回は、**L2スイッチ**と**ルーター**を組み合わせた「Router-on-a-Stick」という手法を用いて、このVLAN間の通信（**VLAN間ルーティング**）を実現します。

ルーターの**単一の物理インターフェース**に複数の論理的なサブインターフェースを設定することで、複数のVLANのトラフィックを効率的に処理します。この設定を構成し、その動作原理を学習することが本プロジェクトの目的です。
