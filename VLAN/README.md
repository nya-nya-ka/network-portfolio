# ルーターとL2スイッチによるVLAN間通信
**VLAN（Virtual LAN）**は、物理的な接続とは無関係に、ブロードキャストドメインを分割し、ネットワークのセキュリティと管理性を高める技術です。

しかし、異なるVLANに属するPC同士は、論理的に分離された別のネットワークと見なされるため、L2スイッチ経由では直接通信できません。

そこで今回は、L2スイッチとルーターを組み合わせた**「Router-on-a-Stick（ルーター・オン・ア・スティック）」という手法を用いて、このVLAN間の通信（VLAN間ルーティング）を実現します。ルーターの単一の物理インターフェース**で複数のVLANのトラフィックを処理するこの効率的な設定を構成し、その動作原理を学習します。
